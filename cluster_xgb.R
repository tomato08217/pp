...
library(readxl)
library(caret)
all<-read_xlsx("part2.xlsx",sheet=1)
...
for (csplit in c(ceiling(runif(7)*300))){
set.seed(csplit)
...
combo<-dfTune%>%filter(train_auc>0.75)%>%filter(test_auc>0.7)%>%filter(ex_auc>0.7)
